...
const enemy =
    new EvilWizard(player, 32000, 31800, 128, 24, 96, 0, 32, 100, 100, {
...
function gameLoop() {
  if (player.health <= 0) {
    const text = textManager.addText('可憐', 'red');
    text.progress = 1;
    textManager.render(ctx);
    return;
  }
  ...
  if (enemy.death) {
    const text = textManager.addText('幸運而已', 'white');
    text.progress = 1;
    textManager.render(ctx);
    return;
  }
...
async function main() {
  await sleep(2000);
  textManager.addText('第一關:綠色的地 (米洛斯有機率躲掉攻擊喔)');
  textManager.addText('Stage 1: Grass Land (Milos have some chance to hide your attack)');
  await sleep(2000);
  textManager.addText('米洛斯香蕉君想蕉個朋友');
  textManager.addText('Milos banana man want to make friend');
  await sleep(1000);
